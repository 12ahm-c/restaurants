...
export function TableMapPage() {
  ...
  const statusConfig: Record<TableStatus, { bg: string; text: string; dot: string; label: string; gradient: string }> = {
    free: { bg: 'bg-emerald-50', text: 'text-emerald-700', dot: 'bg-emerald-500', label: t('tables.free'), gradient: 'from-emerald-400 to-green-500' },
    occupied: { bg: 'bg-red-50', text: 'text-red-700', dot: 'bg-red-500', label: t('tables.occupied'), gradient: 'from-red-400 to-rose-500' },
    reserved: { bg: 'bg-amber-50', text: 'text-amber-700', dot: 'bg-amber-500', label: t('tables.reserved'), gradient: 'from-amber-400 to-orange-500' },
    'in-service': { bg: 'bg-blue-50', text: 'text-blue-700', dot: 'bg-blue-500', label: t('tables.inService'), gradient: 'from-blue-400 to-indigo-500' },
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-display font-bold text-gray-900">{t('nav.tables')}</h1>
          <p className="text-sm text-gray-500 mt-1">{t('tables.manage')}</p>
        </div>
      ...
      {statusSummary && (
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          <div className="card bg-white p-4 text-center">
            <div className="w-10 h-10 bg-emerald-100 rounded-xl flex items-center justify-center mx-auto mb-2"><div className="w-3 h-3 bg-emerald-500 rounded-full" /></div>
            <p className="text-2xl font-bold text-emerald-600">{statusSummary.free}</p>
            <p className="text-xs text-gray-500 font-medium">{t('tables.free')}</p>
          </div>
          <div className="card bg-white p-4 text-center">
            <div className="w-10 h-10 bg-red-100 rounded-xl flex items-center justify-center mx-auto mb-2"><div className="w-3 h-3 bg-red-500 rounded-full" /></div>
            <p className="text-2xl font-bold text-red-600">{statusSummary.occupied}</p>
            <p className="text-xs text-gray-500 font-medium">{t('tables.occupied')}</p>
          </div>
          <div className="card bg-white p-4 text-center">
            <div className="w-10 h-10 bg-amber-100 rounded-xl flex items-center justify-center mx-auto mb-2"><div className="w-3 h-3 bg-amber-500 rounded-full" /></div>
            <p className="text-2xl font-bold text-amber-600">{statusSummary.reserved}</p>
            <p className="text-xs text-gray-500 font-medium">{t('tables.reserved')}</p>
          </div>
          <div className="card bg-white p-4 text-center">
            <div className="w-10 h-10 bg-blue-100 rounded-xl flex items-center justify-center mx-auto mb-2"><div className="w-3 h-3 bg-blue-500 rounded-full" /></div>
            <p className="text-2xl font-bold text-blue-600">{statusSummary.inService}</p>
            <p className="text-xs text-gray-500 font-medium">{t('tables.inService')}</p>
          </div>
          <div className="card bg-white p-4 text-center">
            <div className="w-10 h-10 bg-gray-100 rounded-xl flex items-center justify-center mx-auto mb-2"><Table2 size={20} className="text-gray-500" /></div>
            <p className="text-2xl font-bold text-gray-900">{statusSummary.total}</p>
            <p className="text-xs text-gray-500 font-medium">{t('tables.total')}</p>
          </div>
        </div>
      )}
      ...
      <div className="card bg-white p-6">
            ...
            return (
              <button key={table._id} onClick={() => handleTableClick(table)} disabled={table.status !== 'free'}
                ...
                <div className="mt-2">
                  ...
                  <div className="flex items-center justify-center gap-1 text-xs text-gray-500 mt-1"><Users size={12} /><span>{table.capacity}</span></div>
                ...
                {table.status !== 'free' && (
                  <button onClick={(e) => handleClearTable(table._id, e)} className="absolute top-3 right-3 p-1.5 rounded-lg bg-white/80 hover:bg-white text-red-500 hover:text-red-600 transition-all shadow-sm" title={t('tables.clearTable')}>
                    <Trash2 size={14} />
                  </button>
                )}
              </button>
        ...
        {filteredTables.length === 0 && (
          <div className="text-center py-12">
            <div className="w-16 h-16 bg-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-4"><Table2 size={32} className="text-gray-300" /></div>
            <p className="text-gray-500 font-medium">{t('tables.noTables')}</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-md card p-6 animate-scale-in">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-display font-bold text-gray-900">{t('tables.createTable')}</h3>
              <button onClick={() => setShowModal(false)} className="p-2 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors"><X size={18} /></button>
            </div>
            <form onSubmit={handleCreateTable} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">{t('tables.tableName')} *</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">{t('tables.capacity')} *</label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">{t('tables.zone')} *</label>
