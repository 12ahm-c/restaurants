...
export function SettingsPage() {
  ...
  const [tentPriceSmall, setTentPriceSmall] = useState(0);
  const [tentPriceMedium, setTentPriceMedium] = useState(0);
  const [tentPriceLarge, setTentPriceLarge] = useState(0);
  ...
  useEffect(() => {
    if (settings) {
      ...
      if (settings.tent_price_per_hour) {
        setTentPriceSmall(settings.tent_price_per_hour.small || 0);
        setTentPriceMedium(settings.tent_price_per_hour.medium || 0);
        setTentPriceLarge(settings.tent_price_per_hour.large || 0);
      }
    }
  }, [settings]);
  ...
  const handleSaveGeneral = async (e: React.FormEvent) => {
    ...
    try {
      await updateSettings({
        ...
        tent_price_per_hour: {
          small: tentPriceSmall,
          medium: tentPriceMedium,
          large: tentPriceLarge,
        },
      });
    ...
    <div className="p-4 space-y-4 pb-8">
      ...
      <form onSubmit={handleSaveGeneral} className="card rounded-2xl border dark:border-white/5 border-black/5 shadow-card p-5 space-y-4">
        ...
        <div className="border-t dark:border-white/5 border-black/5 pt-4 mt-4">
          <h3 className="text-md font-semibold dark:text-white text-surface-900 mb-1">{t('settings.tentPricePerHour')}</h3>
          <p className="text-xs dark:text-surface-400 text-surface-500 mb-3">{t('settings.tentPricePerHourHint')}</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-semibold text-surface-300 mb-1.5">{t('settings.tentPricing.small')}</label>
              <input
                type="number"
                value={tentPriceSmall}
                onChange={(e) => setTentPriceSmall(parseFloat(e.target.value) || 0)}
                className="input-field"
                min="0"
              />
            </div>
            <div>
              <label className="block text-sm font-semibold text-surface-300 mb-1.5">{t('settings.tentPricing.medium')}</label>
              <input
                type="number"
                value={tentPriceMedium}
                onChange={(e) => setTentPriceMedium(parseFloat(e.target.value) || 0)}
                className="input-field"
                min="0"
              />
            </div>
            <div>
              <label className="block text-sm font-semibold text-surface-300 mb-1.5">{t('settings.tentPricing.large')}</label>
              <input
                type="number"
                value={tentPriceLarge}
                onChange={(e) => setTentPriceLarge(parseFloat(e.target.value) || 0)}
                className="input-field"
                min="0"
              />
            </div>
