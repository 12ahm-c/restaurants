...
export function FinancePage() {
  ...
  const handleAddExpense = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast('error', error instanceof Error ? error.message : 'Failed to add expense');
    }
  };
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="mt-4 flex items-center space-x-4 text-sm">
            <span className="flex items-center text-gray-500">
              <Banknote size={14} className="mr-1" /> {summary?.incomeByMethod.cash.toLocaleString() || 0}
            </span>
            <span className="flex items-center text-gray-500">
              <CreditCard size={14} className="mr-1" /> {summary?.incomeByMethod.card.toLocaleString() || 0}
            </span>
            <span className="flex items-center text-gray-500">
              <Smartphone size={14} className="mr-1" /> {summary?.incomeByMethod.mobile.toLocaleString() || 0}
            </span>
          </div>
        </div>
        ...
        <div className={`bg-white rounded-lg shadow p-6 ${(summary?.netProfit || 0) >= 0 ? 'border-l-4 border-green-500' : 'border-l-4 border-red-500'}`}>
          ...
          <div className="mt-4">
            <div className="text-sm text-gray-500">
              Profit Margin: {summary?.totalIncome ? ((summary.netProfit / summary.totalIncome) * 100).toFixed(1) : 0}%
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-lg font-semibold mb-4 flex items-center">
              <BarChart3 size={20} className="mr-2" /> Last 7 Days
            </h3>
                    ...
                    <div className="flex-1 space-y-1">
                      <div className="flex items-center">
                        <div
                          ...
                          style={{ width: `${(income / max) * 100}%`, minWidth: income > 0 ? '4px' : '0' }}
                        />
                      ...
                      <div className="flex items-center">
                        <div
                          ...
                          style={{ width: `${(expense / max) * 100}%`, minWidth: expense > 0 ? '4px' : '0' }}
                        />
            ...
            <div className="flex items-center mt-4 space-x-4 text-sm">
              <span className="flex items-center"><div className="w-3 h-3 bg-green-400 rounded mr-1" /> Income</span>
              <span className="flex items-center"><div className="w-3 h-3 bg-red-400 rounded mr-1" /> Expenses</span>
            </div>
          </div>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-500">
                        {new Date(tx.date).toLocaleDateString()} {tx.user && `- ${tx.user}`}
                      </div>
                    </div>
                  </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="divide-y divide-gray-200">
              {(summary?.recentPayments || []).length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-6 py-8 text-center text-gray-500">No income recorded</td>
                </tr>
              ) : (
                (summary?.recentPayments || []).map((payment: any) => (
                  <tr key={payment._id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">
                      #{payment.orderId?.orderNumber || payment._id.slice(-6).toUpperCase()}
                    </td>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., June Rent, Electric Bill"
                  />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Amount (MRU) *</label>
                    <input
                      type="number"
