...
class ReportsService {
    static async getSalesReport(from, to) {
        ...
        const salesData = await Payment_1.Payment.aggregate([
    ...
    static async getProfitabilityReport(from, to) {
        const startDate = new Date(from);
        const endDate = new Date(to);
        endDate.setHours(23, 59, 59, 999);
        ...
        const [revenueResult, expensesResult] = await Promise.all([
            ...
            StockMovement_1.StockMovement.aggregate([
    ...
    static async getStockUsageReport(from, to) {
        const startDate = new Date(from);
        const endDate = new Date(to);
        endDate.setHours(23, 59, 59, 999);
        ...
        const stockUsage = await StockMovement_1.StockMovement.aggregate([
